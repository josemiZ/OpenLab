...
class WelcomeAdapter : BaseAdapter<RecyclerView.ViewHolder, Any>() {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) = with(holder) {
        when (itemViewType) {
            HEADER -> {
                val headerHolder = this as WelcomeHeaderViewHolder
                val item = list[position] as WelcomeHeaderModel
                headerHolder.bind(item)
            }
            NORMAL -> {
                val contentHolder = this as WelcomeViewHolder
    ...
    class WelcomeViewHolder(
        ...
        fun bind(model: WelcomeModel) = with(item) {
            this.model = model
    ...
    class WelcomeHeaderViewHolder(
        ...
        fun bind(model: WelcomeHeaderModel) = with(item) {
            this.model = model
            executePendingBindings()
        }
    }
